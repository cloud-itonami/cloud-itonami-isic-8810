(ns care.operation
  "OperationActor -- one care operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (CareOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Safeguarding Governor (:govern) and the rollout
  ...
  One graph run = one care operation (intake -> advise -> govern ->
  ...
  decision to a human operator (care-coordinator/clinical lead). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/dispatch-checkin`/`:actuation/close-case`
  ALWAYS reach this node when the governor is clean -- see `care.
  phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [care.careadvisor :as careadvisor]
...
(defn build
  ...
  `care.store/Store`).
  opts:
    :advisor      -- a `care.careadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (careadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CareOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (careadvisor/-advise advisor store request)]
            {:proposal p :audit [(careadvisor/trace request p)]})))

      ;; Safeguarding Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
